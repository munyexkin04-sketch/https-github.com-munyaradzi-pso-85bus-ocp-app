...
def load_flow(q_caps, bus_indices=None, system_id='85'):
    """Calculates total power loss (kW) and bus voltages for specified system"""
    ...
    # Map load data to bus arrays
    for row in loads:
        p_load[int(row[0])-1] = row[1]
        q_load[int(row[0])-1] = row[2]
        
    # Apply Capacitor Injections
    ...
    V = np.ones(n_bus)
    pl_loss = np.zeros(n_lines)
    P_br = np.zeros(n_lines)
    Q_br = np.zeros(n_lines)
    
    # BFS Iterations
    for _ in range(4):
        # Calculate Branch Power flows (Backward)
        for i in range(n_lines-1, -1, -1):
            t_bus = int(network[i, 2]) - 1
            P_br[i] = P[t_bus] + np.sum(P_br[network[:, 1] == (t_bus + 1)])
            Q_br[i] = Q[t_bus] + np.sum(Q_br[network[:, 1] == (t_bus + 1)])
            
        # Calculate Voltages (Forward)
        for i in range(n_lines):
            f_bus = int(network[i, 1]) - 1
            t_bus = int(network[i, 2]) - 1
            vs = V[f_bus]
    ...
    return np.sum(pl_loss) * base_kva, V, P_br * base_kva, Q_br * base_kva
